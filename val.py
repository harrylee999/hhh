...
def val(model, val_generator, params, opt, is_training, **kwargs):
    ...
    for iter, data in enumerate(val_loader):
        ...
        if opt.cal_map:
            ...
            seg = torch.zeros((seg_annot.size(0), len(params.seg_list)+1, imgs[0][1].size()[0], imgs[0][2].size()[1]), dtype=torch.int32)
...
if __name__ == "__main__":
    ...
    ap.add_argument('-p', '--project', type=str, default='cell', help='Project file that contains parameters')
    ...
    ap.add_argument('-n', '--num_workers', type=int, default=0, help='Num_workers of dataloader')
    ap.add_argument('--batch_size', type=int, default=2, help='The number of images per batch among all devices')
